    ...
    if(person_type == "student"){
        if(login_flag){
            ...
            for(int i=0; i < list_files.size() - 1; i++)
            {
                ...
                if(file.split('/').at(2) != "" && file.split('/').at(3).split('.')[0].endsWith(class_num.left(class_num.size() - 1) + "_" + Translit->toTranslit(QString(class_num[class_num.size() - 1])))){
        ...
        if(login_flag){
            ...
            foreach(QString num_class, class_num.split(";")){
                ...
                for(int i=0; i < list_files.size() - 1; i++)
                {
                    ...
                    if(file.split('/').at(2) != "" && file.split('/').at(3).split('.')[0].endsWith(num_class.left(num_class.size() - 1) + "_" + Translit->toTranslit(QString(num_class[num_class.size() - 1])))){
                        file_name = file.split('/').at(3);
...
void MainWindow::reciver(){
    ...
    if(data != "ping@!#"){
        ...
        data = data.replace("#$#", "\n");
        user = data.split('%')[0];
        user = user.left(user.size() - 1);
        QString lesson = data.split('%')[1];
        lesson = lesson.left(lesson.size() - 2).right(lesson.size() - 3);
        if(data.split('%').size() >= 3){
            ...
            file.write(QString(time.toString("hh:mm:ss") + ":&" + data.left(user.size() + lesson.size() + 7).replace('\n', "#$#") + "\n").toLocal8Bit());
            if(!lock && ui->excercises->count() != 0 && ui->excercises->currentItem()){
                if(!lock && lesson.split('@')[0] == Translit->toTranslit(current_lesson) && lesson.split('@')[1].replace("_", " ") == Translit->toTranslit(ui->excercises->currentItem()->text())){
                    ...
                    ui->chat_viewer->addItem("[" + time.toString("hh:mm:ss") + "] " + user + ": " + data.left(user.size() + lesson.size() + 7));
                }else {
