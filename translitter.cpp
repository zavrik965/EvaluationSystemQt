...
QString Translitter::toTranslit(QString line)
{
    ...
    {
        if(russian_upper.filter(QString(line[i])).size() != 0 || russian_lower.filter(QString(line[i])).size() != 0){
            if(line[i].isLower() && line[i].isLetter())
            {
                letter = translit_lower[russian_lower.indexOf(QString(line[i]))];
            } else if(line[i].isUpper() && line[i].isLetter())
            {
                letter = translit_upper[russian_upper.indexOf(QString(line[i]))];
            }
        } else{
            letter = line[i];
        }
...
QString Translitter::fromTranslit(QString line)
{
    ...
    while(index >= 0){
        ...
        {
            if(line[index].isUpper()) {letter = russian_upper[translit_upper.indexOf(QString(line[index]))];}
            else if(translit_lower.indexOf(QString(line[index])) != -1) {letter = russian_lower[translit_lower.indexOf(QString(line[index]))];}
            else {letter = line[index];}
            detranslit_line = letter + detranslit_line;
        } else if(line[index] != 'h' && (index == 0 || spec_symbols.filter(QString(line[index - 1])).size() == 0))
        {
            letter = russian_lower[translit_lower.indexOf(QString(line[index]))];
            detranslit_line = letter + detranslit_line;
        } else
        {
